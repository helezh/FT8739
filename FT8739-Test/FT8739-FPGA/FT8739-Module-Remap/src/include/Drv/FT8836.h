...
/* CGUCON0 */
//SBIT HW_MON_GATE_EN       = CGUCON0^7;  /*ASM时钟，在hw mon时是否进行时钟门控。1:门控.在hw mon时,扫描开启asm时钟,扫描结束关闭时钟;0:不门控.hw mon时，asm时钟一直打开 */
...
//SBIT HW_SCAN_MCU_STA      = CGUCON0^0;  /* hw mon时，tp进行扫描，mcu所处状态.0:mcu固定为inactive状态;1:mcu为active状态之前状态*/
...
//SBIT HW_MON_KEEP          = CGUCON1^4;     /* Hw monitor时，mcu在扫描时，是否保持进入hardware monitor之前的状态.0:mcu保持standy;1:mcu保持进入hw mon之前的状态 */
