...
#define DMA_CLR_RI_DIS           ((UINT16)1<<2)

#define DMA_CLR_TI_EN            ((UINT16)1<<1)
#define DMA_CLR_TI_DIS           ((UINT16)1<<1)
...
* Return:
*******************************************************************************/
void SPI0_DMA_SetAddr(UINT32 start_addr,UINT32 end_addr,UINT16 mem_sel)
{    
...
void Test_DMA_MEM(void)
{
  ...
  /******************************************/
    ...
    SPI0_DMA_Init();
    SPI0_DMA_SetAddr((UINT32)ucpAsmram,(UINT32)(ucpAsmram+5),DMA_MEM_ARAM);
...
void Test_DMA_HS(void)
{
  ...
  /******************************************/
    ...
    SPI0_DMA_SetAddr((UINT32)ucpAsmram,(UINT32)(ucpAsmram+0X04ff),DMA_MEM_ARAM);
...
//#else        
        ...
        DMA_CLR_RI_EN|DMA_CLR_TI_EN|DMA_EN));
        DBG_SPI("\n test dma read aram");
        SPI1_Read(PRAM2_PTR8(0X500),0X500);
        ...
        for (i = 0; i < 0X500; i++)
        {
            if (PRAM2_ADDR8(i) != PRAM2_ADDR8(0X500+i))
            {
                DBG_SPI("\ntest err\n");
                HOLD;
            }
            ...
            //DBG_SPI("%02x ",PRAM2_ADDR8(0X500+i));            
