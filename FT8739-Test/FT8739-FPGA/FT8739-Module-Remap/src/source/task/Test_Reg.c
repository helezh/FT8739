...
void Test_AsmReg(void)
{
    ...
    Test_WriteRamReg(0xB120,0xBFFF,0x0000);//hw_mon_ctr0
    Test_WriteRamReg(0xB121,0x9FFF,0x0000);//hw_mon_ctr1    
...
void Test_CalReg(void)
{
    ...
    // 0x0B -- 0x17 为只读，不需要测试
    Test_WriteRamReg(0xB000,0x6ff0,0x0000);
    ...
    // 0x1B为只读，不需要测试
    Test_WriteRamReg(0xB01c,0x7f18,0x0000);//bit2-bit0为WO，读该位MCU会挂掉
    // 0x1D为只读，不需要测试

    for(Addr = 0xB01e; Addr < 0xB027; Addr++)
    {
        Test_WriteRamReg(0xB01e,0xffff,0x0000);
    }
