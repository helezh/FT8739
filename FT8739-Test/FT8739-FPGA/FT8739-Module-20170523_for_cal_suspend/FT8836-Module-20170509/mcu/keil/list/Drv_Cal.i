...
#line 22 "..\..\src\source\driver\Drv_Cal.c" 
 ...
 ST_CalRegisters *pCalRegs = (volatile ST_CalRegisters *)0xB000;
 ...
 UINT8 g_ucCalStartFlag = 0;
 UINT8 g_ucAmoveStart = 0;
 ...
 pCalRegs->rDmaLen = ((1 << 5) + 1);
 
 pCalRegs->rTxRxNum = (ucTxNum << 8) + ucRxNum;        
 pCalRegs->rTxRxNumAm = (ucTxNum << 8)+ ucRxNum;       
 
 
 pCalRegs->rDmask = usDmask; 
 pCalRegs->rDmaskA = usDmaskA; 
 ...
 pCalRegs->rCmd |= (cmd|((UINT16)1<<(0)));                       
 g_ucCalStartFlag = 1;
 P0_7 = 1;
 
 
 while((pCalRegs->rInt&((UINT16)1<<(15))) != ((UINT16)1<<(15)))           
 {
 
 
 
 if((pCalRegs->rCmd&((UINT16)1<<(3))) == ((UINT16)1<<(3)))  
 {    
 printf("\n\rSUSPEND=%d\n",((UINT16)1<<(15)));
 pCalRegs->rCmd |= ((UINT16)1<<(2));                    
 g_bCalResumeFlag = 1;
 }
 }
 g_ucCalStartFlag = 0;
 P0_7 = 0;
 pCalRegs->rCmd |= ((UINT16)1<<(14));
 pCalRegs->rCmd  = 0; 
 ...
 pCalRegs->rPar    = (0<<8) + pMcPDP->ucMaxPeakNum;
...
#line 452 "..\..\src\source\driver\Drv_Cal.c" 
...
#line 514 "..\..\src\source\driver\Drv_Cal.c" 
...
#line 576 "..\..\src\source\driver\Drv_Cal.c" 
...
#line 676 "..\..\src\source\driver\Drv_Cal.c" 
 ...
 if(pMcMP->ucCfg == 0)
 {
 pCalRegs->rInt&=(~((UINT16)1<<(14)));             
 
 
 pCalRegs->rNThd = ((Thr<<8)|(Cfg));          
 }
 else if(pMcMP->ucCfg == 1) 
 {
 pCalRegs->rInt|= ((UINT16)1<<(14));               
 ...
 UINT16 ucCalCmd   = pCalRegs ->rCmd;
 
 CALCKEN = 1; 
 
 pCalRegs ->rCmd &= 0x7fff ; 
 
 pCalRegs->rSizeAm = (pMcMP->ucS1TxLen << 8) + pMcMP->ucS1RxLen;   
 pCalRegs->rS1AddrA = pMcMP->usSrc1Addr;    
 pCalRegs->rOutAddrA = pMcMP->usDstAddr;    
 pCalRegs->rInt &= 0xFF00;
 pCalRegs->rInt |= (0<<3); 
 pCalRegs->rInt |= ((UINT16)1<<(4)); 
 pCalRegs->rInt |= ((UINT16)1<<(0)); 
 
 
 while ((pCalRegs->rInt&((UINT16)1<<(7))) != ((UINT16)1<<(7)))
 {
 printf("\nwaiting...int");
 
 if ((pCalRegs->rInt&((UINT16)1<<(5))) == ((UINT16)1<<(5)))
 {		
 pCalRegs->rInt |= ((UINT16)1<<(1));
 g_bAmoveResumeFlag = 1;
 }
 }
 g_ucAmoveStart = 0;
 
 pCalRegs->rInt |= ((UINT16)1<<(2));
 pCalRegs->rInt &= 0xFF00;
 pCalRegs->rCmd = ucCalCmd;
 
 if(g_bAmoveResumeFlag)
 {
 printf("\n\n\nAmove suspend");
 printf("\nAmove suspend");
 printf("\nAmove suspend");
 g_bAmoveResumeFlag = 0;
 }
 ...
 pCalRegs->rSize   = (pMcMP->ucTxLen<<8) + pMcMP->ucRxLen;
 pCalRegs->rInt &= (~(1<<14));
 pCalRegs->rInt |= (pMcMP->ucDataType<<14);                                 
 
 pCalRegs->rCmd &= (~(1<<8));    
 pCalRegs->rCmd |= ((UINT16)1<<(15)) + ((pMcMP->ucCfg & 0x01)<<8);         
