...
/*F0-F7*/
SFR CNT_MASK            = 0xF1; /*  7~4
                                    4'hA:写rst_cnt_clr为A可清零cpu_xxx_cnt
                                    
                                    3~0
                                    4'h0:porrst_cnt;（可被外部复位信号复位）
                                    4'h1:wdtrst_cnt;（可被外部复位信号复位）
                                    4'h2:extrst_cnt;（可被LCD复位信号复位）
                                    4'h3:lcdrst_cnt;（可被外部复位信号复位）
                                    4'b4:esd_cnt;   （可被外部复位信号复位）
                                    4'h5:lvd_cnt;   （可被外部复位信号复位）
                                    4'h8:cpu_porrst_cnt;（可被CPU写rst_cnt_clr清零）
                                    4'h9:cpu_wdtrst_cnt;（可被CPU写rst_cnt_clr清零）
                                    4'hA:cpu_extrst_cnt;（可被CPU写rst_cnt_clr清零）
                                    4'hB:cpu_lcdrst_cnt;（可被CPU写rst_cnt_clr清零）
                                    4'bC:cpu_esd_cnt;   （可被CPU写rst_cnt_clr清零）
                                    4'hD:cpu_lvd_cnt;   （可被CPU写rst_cnt_clr清零）
                                    */      
SFR CNT_STA             = 0xF2; /* 根据cnt_mask的值，读出不同的cnt值*/
SFR ANACON10            = 0xF3;
SFR SPI0CON6            = 0xF4;
SFR SSCG_CON0           = 0xF5;
...
/*SPI0CON*/
SBIT DMA2_END_FLAG          = SPI0CON^7;  /*DMA2操作结束中断请求标志位，软件清零*/
...
SBIT SPI0_STOP_EN           = SPI0CON2^3; /*spi_stop中断请求信号使能*/
...
/*SPI0CON5*/
SBIT DMA2_END_CLR           = SPI0CON5^7;  /*写1清除dma2 end flag*/
...
/*SPI0CON6*/
SBIT SPI0_WK_CLR            = SPI0CON6^0;  /*写1清除SPI0 WAKEUP*/
...
SBIT UART_REN1              = UARTCON^4; /*接收使能控制位.1:允许接收;0:禁止接收*/
...
SBIT ISP_MON_INT            = IWKSTA^1; /* i2c monitor/i2c wakeup/spi0 wakeup中断唤醒标志 */
...
/*SFTRST[0] */              /*当全写1时产生软复位;全写0时产生remap复位;当读时是软复位标志*/
...
/* ANACON10 */
SBIT LVD_VDDI_EN            = ANACON10^2; /*1:开启vddi检测;0:关闭vddi检测*/
SBIT LVD_AVEE_EN            = ANACON10^1; /*1:开启avee检测;0:关闭avee检测*/
SBIT LVD_AVDD_EN            = ANACON10^0; /*1:开启avdd检测;0:关闭avdd检测*/
...
SBIT ISP_MON_WKEN           = CGUCON5^1;  /* i2c wakeup/i2c monitor/spi0 wakeup enable */
...
//SBIT SPI0_STOP_EI       = AIE^0;    /* spi0 stop int enable */ 
SBIT ISP_STOP_EI        = AIE^0;    /* i2c stop and spi0 stop int enable */
