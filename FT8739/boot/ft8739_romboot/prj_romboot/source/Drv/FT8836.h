...
SBIT SPI0_CS_DEGLITCH_EN    = SPI0CON3^6; /**0*******/
SBIT SPI0_PHASE             = SPI0CON3^5; /**1*******/
...
SBIT ISP_STOP_IF        = AIF^0;      /* isp stop int请求信号 */
...
SBIT ISP_STOP_EI        = AIE^0;    /* isp stop int enable */  
...
SBIT ISP_STOP_IPH       = AIPH^0;   /* ISP Stop */
SBIT ISP_STOP_IPL       = AIPL^0;
...
typedef enum
{
    ...
    FW_CFG_LCD_CNT_OFFSET          = 0x0A,     // LCD reload cnt
    FW_CFG_LCD_CNT_NE_OFFSET       = 0x0B,     // ~LCD reload cnt
    FW_CFG_SPI_IOMODE_OFFSET       = 0x0C,     // spi iomode
    FW_CFG_SPI_IOMODE_NE_OFFSET    = 0x0D,     // ~spi iomode
    FW_CFG_SPI0_PHASE_OFFSET       = 0x0E,     // spi0 phase
    FW_CFG_SPI0_PHASE_NE_OFFSET    = 0x0F,     // ~spi0 phase
} ENUM_FW_CFG_OFFSET;
...
#define DRAM_PTR8(n)     ((UINT8 far  *) ((UINT32)(n)))
